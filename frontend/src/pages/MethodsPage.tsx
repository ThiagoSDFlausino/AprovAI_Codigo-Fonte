...
const MethodCard = ({ method, isAdmin, onEdit, onDelete }) => {
  ...
  const catLabel = StudyMethodController.getCategoryLabel(method.category);
...
const MethodsPage = () => {
  ...
  const categories = StudyMethodController.getCategories();

  const load = useCallback(() => {
    StudyMethodController.PesquisaMetodoEstudo({ search, category }, setMethods, setLoading, toast);
  }, [search, category]);
  ...
  const handleSave = async (formData) => {
    ...
    if (modal.mode === 'create') {
      await StudyMethodController.CadastroMetodoEstudo(formData, user.id, setMethods, () => setModal(null), toast);
    } else {
      await StudyMethodController.AtualizarMetodoEstudo(modal.method.id, formData, setMethods, () => setModal(null), toast);
    }
  };

  const handleDelete = async (method) => {
    await StudyMethodController.DeletarMetodoEstudo(method.id, setMethods, toast);
